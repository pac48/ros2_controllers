...
controller_interface::return_type AdmittanceRule::configure(std::shared_ptr<rclcpp_lifecycle::LifecycleNode> node, int num_joints)
{
    ...
    std::vector<double> desired_ee_vel(6);
    ...
    try
    {
      ...
      // the next line from "admittance_controller" to "ik_base_plugin"
        ik_loader_ = std::make_shared<pluginlib::ClassLoader<kinematics_interface::KinematicsBaseClass>>(
                "kinematics", "kinematics_interface::KinematicsBaseClass");
...
{
// TODO need to set everything back to zero e.g pos.assign(0)

// base_T_ee    base_T_force   => ee_T_force =   inv(base_T_ee)*base_T_force = base_T_ee*inv(base_T_force)
...
//                                  measured_wrench.torque.x*0, measured_wrench.torque.y*0, measured_wrench.torque.z*0};
    ...
    // TODO fix this ^^
        ...
        for(int i =0; i < 6; i++){
            tmp[i] = reference_joint_state.velocities[i];
        }

        if (!ik_->convert_joint_deltas_to_cartesian_deltas(
                tmp, identity_transform_, desired_ee_vel) ){
...
//            pose_error[axis] = -(cur_ee_pos[axis] - desired_ee_pos[axis]) - current_pose_arr_[axis];   not terrible
            pose_error[axis] = -current_pose_arr_[axis];
            ...
            admittance_acceleration[axis] = (1.0 / parameters_.mass_[axis]) * (wrench[axis] +
                    (parameters_.damping_[axis] * (desired_ee_vel[axis] - admittance_velocity_arr_[axis])) +
                    ...
                    current_pose_arr_[axis] += admittance_velocity_arr_[axis]*(1.0 / 1000);
        }
    }
    ...
    for(int i =0; i < 6; i++){
        ...
        if (!ik_->convert_cartesian_deltas_to_joint_deltas(tmp, identity_transform_, joint_vel)
                || !ik_->convert_cartesian_deltas_to_joint_deltas(admittance_acceleration, identity_transform_, joint_acc)
                || !ik_->convert_cartesian_deltas_to_joint_deltas(wrench, identity_transform_, joint_torques))
        {
            RCLCPP_ERROR(rclcpp::get_logger("AdmittanceRule"),
                         "Conversion of joint deltas to Cartesian deltas failed. Sending current joint"
                         " values to the robot.");
        ...
        {
            pos[j] += joint_vel[j]*(1.0/1000.0) - .2*pos[j]*(1.0/1000.0) ;
            // Store data for publishing to state variable
            desired_joint_state.positions[j] = reference_joint_state.positions[j] + pos[j];
...
{
    measured_wrench_.wrench.force.x = filters::exponentialSmoothing(measured_wrench.force.x, measured_wrench_.wrench.force.x, alpha);
    measured_wrench_.wrench.force.y = filters::exponentialSmoothing(measured_wrench.force.y, measured_wrench_.wrench.force.y, alpha);
    measured_wrench_.wrench.force.z = filters::exponentialSmoothing(measured_wrench.force.z, measured_wrench_.wrench.force.z, alpha);

    measured_wrench_.wrench.torque.x = filters::exponentialSmoothing(measured_wrench.torque.x, measured_wrench_.wrench.torque.x, alpha);
    measured_wrench_.wrench.torque.y = filters::exponentialSmoothing(measured_wrench.torque.y, measured_wrench_.wrench.torque.y, alpha);
    measured_wrench_.wrench.torque.z = filters::exponentialSmoothing(measured_wrench.torque.z, measured_wrench_.wrench.torque.z, alpha);
